...
export async function POST(request: Request) {
  ...
  const fetchCodeByValue = async (value: string) => {
    ...
    codeUrl.searchParams.set(
      "select",
      "id,diagnostic_id,issued_to_name,issued_to_email,status"
    );
  ...
  const email = normalize(codeRow.issued_to_email);
  const name = normalize(codeRow.issued_to_name);
  const hasIdentifier = Boolean(email || name);

  if (hasIdentifier) {
    if (!identifier || (identifier !== email && identifier !== name)) {
      return new Response("The email or name does not match this access code.", { status: 401 });
    }
  }
