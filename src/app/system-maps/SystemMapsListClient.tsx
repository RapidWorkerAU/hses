...
import { hasMapCategoryAccess } from "@/components/dashboard/portal/dashboardPortals";
import shellStyles from "./SystemMapsListClient.module.css";
...
function LinkMapCodeControl({
  ...
  return (
    <div className={shellStyles.linkControl}>
      <button
        type="button"
        className={`${shellStyles.button} ${shellStyles.buttonSecondary}`}
        onClick={() => onOpenChange(!open)}
      >
        <Image src="/icons/relationship.svg" alt="" width={16} height={16} className={shellStyles.buttonIcon} />
        {label}
      </button>
      {open ? (
        <form
          className={shellStyles.linkPanel}
          onSubmit={(event) => {
            event.preventDefault();
            onSubmit();
          }}
        >
          <label className={shellStyles.linkLabel} htmlFor="system-map-code">
            Map code
          </label>
          <div className={shellStyles.linkInputRow}>
            <input
              id="system-map-code"
              value={value}
              onChange={(event) => onValueChange(event.target.value)}
              className={shellStyles.linkInput}
              placeholder="Enter shared map code"
              autoComplete="off"
            />
            <button type="submit" className={`${shellStyles.button} ${shellStyles.buttonAccent}`} disabled={busy}>
              {busy ? "Linking..." : "Link"}
            </button>
          </div>
        </form>
...
export default function SystemMapsListClient() {
  ...
  const loadMaps = useCallback(async () => {
    ...
    const memberMapIds = [...memberByMapId.keys()];

    const mapRowsById = new Map<string, Omit<SystemMapRow, "role">>();
    if (memberMapIds.length) {
      ...
      ((data ?? []) as Omit<SystemMapRow, "role">[]).forEach((row) => mapRowsById.set(row.id, row));
    }

    const { data: ownedRows, error: ownedMapsError } = await supabaseBrowser
      .schema("ms")
      .from("system_maps")
      .select("id,title,description,owner_id,map_code,map_category,updated_at,created_at")
      .eq("is_template_editor", false)
    ...
    if (ownedMapsError) {
      setError(ownedMapsError.message || "Unable to load your system maps.");
      return;
    }

    ((ownedRows ?? []) as Omit<SystemMapRow, "role">[]).forEach((row) => mapRowsById.set(row.id, row));

    const mergedRows = [...mapRowsById.values()]
      .map((row) => ({
        ...row,
        role: row.owner_id === user.id ? "full_write" : memberByMapId.get(row.id)?.role ?? "read",
      }))
      .filter((row) => hasMapCategoryAccess(user.email ?? null, row.map_category))
  ...
  if (isLoading) {
    return (
      ...
      >
        <TableSkeleton rows={pageSize} columns="18% 22% 12% 10% 10% 12% 12% 12%" showToolbar />
      </HsesDashboardShell>
    );
  }
  ...
  return (
    ...
    >
      <section className={shellStyles.accountCard}>
        ...
        <PortalTableFooter total={rows.length} page={safePage} pageSize={pageSize} onPageChange={setPage} label="maps" />
      </section>
