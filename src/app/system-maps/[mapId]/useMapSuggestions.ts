...
export function useMapSuggestions({
  ...
  const handleRunSuggestionCheck = useCallback(async () => {
    ...
    const userPrompt = suggestionPrompt.replace(/\s+/g, " ").trim();
    if (!userPrompt) {
      setSuggestionError("Ask a question or enter a statement first.");
      return;
    }
    ...
    try {
      ...
      setSuggestionError(
        suggestionRunError instanceof Error ? suggestionRunError.message : "Unable to answer the request."
      );
  ...
  }, [buildSuggestionSnapshot, canUseSuggestionCheck, map, mapId, suggestionPrompt]);
