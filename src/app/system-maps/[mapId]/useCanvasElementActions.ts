...
import type { CanvasElementRow, DocumentNodeRow } from "./canvasShared";
...
type UseCanvasElementActionsParams = {
  ...
  processFillModeDraft: "fill" | "outline";
  processOutlineColorDraft: string | null;
  processOutlineWidthDraft: string;
  ...
  stickyBackgroundColorDraft: string;
  stickyOutlineColorDraft: string;
  stickyFillModeDraft: "fill" | "outline";
  stickyOutlineWidthDraft: string;
  ...
  textBoxOutlineColorDraft: string;
  textBoxOutlineWidthDraft: string;
  selectedTableId: string | null;
  tableRowsDraft: string;
  tableColumnsDraft: string;
  tableHeaderBgDraft: string;
  tableGridLineColorDraft: string;
  tableGridLineWeightDraft: string;
  ...
  flowShapeOutlineColorDraft: string;
  flowShapeOutlineWidthDraft: string;
...
export function useCanvasElementActions(params: UseCanvasElementActionsParams) {
  ...
  const handleAddBlankDocument = useCallback(async () => {
    ...
    // Refs intentionally hold the latest persisted coordinates without forcing a render.
  ...
  const handleSaveProcessHeading = useCallback(async (closeAfterSave = true) => {
    ...
    const parsedOutlineWidth = Number(processOutlineWidthDraft.trim());
    ...
    const nextConfig = {
      ...
      outline_color: normalizeColorHex(processOutlineColorDraft ?? null),
  ...
  const handleSaveStickyNote = useCallback(async (closeAfterSave = true) => {
    ...
    }).eq("id", selectedStickyId).eq("map_id", mapId).select(canvasElementSelectColumns).single();
  ...
  const handleSaveTextBox = useCallback(async (closeAfterSave = true) => {
    ...
    const outlineWidthRaw = Number(textBoxOutlineWidthDraft.trim());
    const outlineWidth = Number.isFinite(outlineWidthRaw) ? Math.max(1, Math.min(12, Math.round(outlineWidthRaw))) : 2;
    const { data, error: e } = await supabaseBrowser
      .schema("ms")
      .from("canvas_elements")
      .update({
        ...
        element_config: {
          ...
          outline_color: normalizeColorHex(textBoxOutlineColorDraft) ?? "#111827",
          outline_width: outlineWidth,
        },
      })
  ...
  const handleSaveTable = useCallback(async (closeAfterSave = true) => {
    ...
    const gridLineColor = normalizeColorHex(tableGridLineColorDraft);
    const parsedGridLineWeight = Number(tableGridLineWeightDraft.trim());
    const gridLineWeight = Number.isFinite(parsedGridLineWeight) ? Math.max(0.5, Math.min(6, Math.round(parsedGridLineWeight * 2) / 2)) : 0.5;
    ...
    const { data, error: e } = await supabaseBrowser
      ...
      .update({
        ...
        element_config: {
          ...
          grid_line_weight: gridLineWeight,
          bold: tableBoldDraft,
  ...
  const handleSaveFlowShape = useCallback(async (closeAfterSave = true) => {
    ...
    const outlineWidthRaw = Number(flowShapeOutlineWidthDraft.trim());
    const outlineWidth = Number.isFinite(outlineWidthRaw) ? Math.max(1, Math.min(12, Math.round(outlineWidthRaw))) : 3;
    ...
    const payload: Partial<CanvasElementRow> = {
      ...
      element_config: {
        ...
        outline_color: normalizeColorHex(flowShapeOutlineColorDraft) ?? normalizedColor,
        outline_width: outlineWidth,
